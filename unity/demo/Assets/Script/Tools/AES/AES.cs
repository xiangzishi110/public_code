...
public class Aes  
{
	static private Aes s_Instance = null;
	static public Aes Instance
	{
		get
		{
			if( null == s_Instance )
			{
				byte[] keyBytes = new byte[] {	0xa0, 0xb1, 0xc2, 0xd3, 0xe4, 0xf5, 0x16, 0x27,
												0x38, 0x49, 0x5a, 0x6b, 0x7c, 0x8d, 0x9e, 0x0f,
												0x90, 0x81, 0x72, 0x63, 0x54, 0x45, 0x36, 0x27};				
				s_Instance = new Aes(KeySize.Bits192, keyBytes);
			}
			//
			return s_Instance;	
		}
    ...
    public const int AES_CODE_LEN = 16;
  ...
  public Aes(KeySize keySize, byte[] keyBytes)
  {
	...
	// Encode
	public	void	Encode( byte[] input, int iEncodeLen, byte[] output )
	{
		if( null == input || null == output || output.Length < iEncodeLen )
			return;
		
		if( iEncodeLen < 16 )
		{
			System.Buffer.BlockCopy( input, 0, output, 0, iEncodeLen );
			return;
		}
		
		//
		int	iModLen = iEncodeLen % 16;
		int	iEnLen	= iEncodeLen - iModLen;
		for( int iLen = 0; iLen < iEnLen; iLen += 16 )
		{
			Cipher( input, output, iLen, iLen );
		}
		
		//
		if( 0 != iModLen )
		{
			System.Buffer.BlockCopy( input, iEnLen, output, iEnLen, iModLen );
		}
	}
	
	// Decode
	public	void	Decode( byte[] input, int iDecodeLen, byte[] output )
	{
		if( null == input || null == output || output.Length < iDecodeLen )
			return;
		
		if( iDecodeLen < 16 )
		{
			System.Buffer.BlockCopy( input, 0, output, 0, iDecodeLen );
			return;
		}
		
		//
		int	iModLen = iDecodeLen % 16;
		int	iEnLen	= iDecodeLen - iModLen;
		for( int iLen = 0; iLen < iEnLen; iLen += 16 )
		{
			InvCipher( input, output, iLen, iLen );
		}
  ...
  private void Cipher(byte[] input, byte[] output, int iSrcOffset, int iDstOffset)  // Cipher 16-bit input
  {
    // state = input
    this.State = new byte[4,Nb];  // always [4,4]
    for (int i = 0; i < (4 * Nb); ++i)
    {
      this.State[i % 4, i / 4] = input[i + iSrcOffset];
    }
    ...
    for (int i = 0; i < (4 * Nb); ++i)
    {
      output[i + iDstOffset] = this.State[i % 4, i / 4];
    }

  }  // Cipher()

  private void InvCipher(byte[] input, byte[] output, int iSrcOffset, int iDstOffset)  // Decipher 16-bit input
  {
    // state = input
    this.State = new byte[4,Nb];  // always [4,4]
    for (int i = 0; i < (4 * Nb); ++i)
